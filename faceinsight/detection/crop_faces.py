...
def worker(image_path, output_class_dir, args):
        ...
        try:
            ...
            if nrof_faces>0:
                ...
                for i, det in enumerate(det_arr):
                    ...
                    if args.detect_multiple_faces:
                        output_filename_n = "{}_{}{}".format(filename_base, i,
                                                             file_extension)
                    else:
...
def main(args):
    ...
    for cls in ds:
        ...
        p.map(partial(worker, output_class_dir=output_class_dir, args=args),
              cls.image_paths)

    cropped_ds = get_dataset(output_dir, has_class_directories=True)
    total_cropped_img_num = 0
    for cls in cropped_ds:
        total_cropped_img_num += len(cls) 
    print('Total number of images: %d' % total_img_num)
    print('Number of cropped images: %d' % total_cropped_img_num)
...
def parse_arguments(argv):
    ...
    parser.add_argument('--nprocessors',
                        type=int,
                        default=2,
